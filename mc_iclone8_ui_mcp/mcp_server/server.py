...
class MCPServer:
    ...
    def tools(self) -> list[dict[str, Any]]:
        return [
            ...
            {"name": "scene.read_modify", "description": "Lit le sous-arbre borné du panneau Modify en lecture seule.", "inputSchema": {"type": "object", "properties": {"max_elements": {"type": "integer", "minimum": 1, "maximum": 500, "default": 200}, "max_depth": {"type": "integer", "minimum": 1, "maximum": 6, "default": 5}}}},
    ...
    def call(self, name: str, args: dict[str, Any]) -> dict[str, Any]:
        ...
        elif name == "scene.read_modify":
            try:
                state = self.accessibility_reader.read_bounded_descendants(":/AttributeModify.ui", int(args.get("max_elements", 200)), int(args.get("max_depth", 5)))
                result = ToolResult("ok", "scene.read_modify", "Modify", observed_state_before=before, observed_state_after=state, verification={"read_only": True, "element_count": state["element_count"]}, next_step="Comparer ce sous-arbre avant/après une sélection et compléter par capture visuelle.")
            except AccessibilityUnavailable as exc:
                result = ToolResult("blocked", "scene.read_modify", "Modify", observed_state_before=before, warnings=[str(exc)], next_step="Vérifier que le panneau Modify est visible.")
